//! In-process token-bucket rate limiter, keyed by client address.
//!
...
impl RateLimiter {
    ...
    pub fn check(&self, key: IpAddr, now: DateTime<Utc>) -> Decision {
...
mod tests {
    ...
    fn allows_up_to_capacity_then_denies() {
        ...
        assert!(matches!(limiter.check(ip(1), at(0)), Decision::Deny { .. }));
    }
    ...
    fn idle_entries_are_pruned_once_the_map_is_large() {
        ...
        for octet in 0..=255u8 {
            for third in 0..=4u8 {
                limiter.check(IpAddr::from([198, 51, third, octet]), at(0));
            }
        }
