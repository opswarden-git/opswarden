...
export function StepVerification({ data }: StepProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (currentIdx < CONSOLE_LOGS.length) {
        setLogs(prev => [...prev, `[${new Date().toLocaleTimeString()}] ${CONSOLE_LOGS[currentIdx]}`]);
        currentIdx++;
      } else {
  ...
  return (
    <div className="space-y-6 max-w-sm mx-auto w-full">
      <div
        ref={containerRef}
        className="w-full bg-black/60 border border-white/5 rounded-md p-4 font-mono text-xs text-green-500 h-64 overflow-y-auto space-y-1.5 scrollbar-thin scrollbar-thumb-muted/10"
      >
        ...
        {logs.length < CONSOLE_LOGS.length && (
