...
| Lifecycle contract              | Implemented | [ADR 0002](adr/0002-alertmanager-lifecycle-contract.md), per-alert firing/resolved events and mixed-state tests | Merge through the required gate                                             |
| Semantic idempotency            | Implemented | Versioned transition identity and formatting/lifecycle retry tests                                              | Observe retry ratios after rollout                                          |
| HTTP/security matrix            | Implemented | 1 MiB, authentication, provider mismatch, disabled rules, tenant isolation and persisted reaction failure tests | Add mutation tests                                                          |
| Metrics                         | Implemented | Accepted, rejected, duplicate, ignored and failed counters at `GET /metrics`                                    | Add production scrape/dashboard and alerts                                  |
| Real Alertmanager E2E           | Implemented | Alertmanager `v0.32.1` pinned by digest; firing and resolved path runs in CI/release E2E                        | Record post-deploy smoke evidence                                           |
...
| Operator documentation          | Implemented | Setup, contract, metrics, rotation and troubleshooting are documented                                           | Verify token rotation/rollback in a deployed environment                    |
...
### P0 — Merge and operational confidence

1. Run the full required gate and merge this branch through a pull request.
2. Verify the production Prometheus scrape for `/metrics`.
3. Add a small dashboard and alerts for rejected/failed spikes and duplicate
   ratio.
4. Exercise token rotation and rollback in a non-production deployment.
5. Record a credential-free post-deploy firing/resolved smoke report.
...
| File                                              | Baseline lines | Suggested split                                                  |
| ------------------------------------------------- | -------------: | ---------------------------------------------------------------- |
...
| `server/src/domain/automation_catalog.rs`         |            730 | remaining provider and reaction modules                          |
...
| `client-web/components/automations/RulesView.tsx` |            638 | forms, filters, validation, hooks, presentation                  |
