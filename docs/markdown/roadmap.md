...
- **Corps** (optionnel) : le _pourquoi_, pas le _quoi_ — le diff dit déjà le quoi.
...
**Lien avec la release** : chaque PR est _squashée_ → le message de squash (conventionnel) devient l'unique commit sur `main` et alimente le changelog auto du workflow release. Soigner le titre de PR = soigner le changelog.
...
- `docker-compose.yml` v0 : `server` (8080) + `db` (Postgres) qui démarrent _healthy_.
...
- [x] Parcours signup → login → `/me` → logout (token mort après). _(✅ API Server prête)_
...
> `grading-criteria.md` est la grille **héritée** des modules T-JSF-600 / T-DEV-600, avec des libellés **RTC** (_server_, _channel_, _typing_). VIGIL est noté sur **son propre** périmètre (core/extended du sujet, détaillé en §4) **plus** les achievements transverses hérités ci-dessous. Cette table mappe ces transverses au sprint où on les gagne, et retraduit les libellés chat.
...
_Roadmap v1 — à réviser en fin de chaque sprint (rétro solo de 30 min : qu'est-ce qui a marché, qu'est-ce qu'on coupe)._
